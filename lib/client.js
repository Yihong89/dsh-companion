...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SpeakToggle(props) {
      ...
      //
      // De-dup cursor: persisted per session in localStorage so switching
      // sessions (or reloading) never re-reads an old message as "new".
      var spokenMsgRef = React.useRef(null)
      var pendingSpeakRef = React.useRef(null)
      var SPOKEN_CURSOR_KEY = 'dsh-sister.spoken-cursor'
      var readCursor = function () {
        try {
          var raw = window.localStorage.getItem(SPOKEN_CURSOR_KEY)
      ...
      var writeCursor = function (sessionId, seq) {
        try {
          window.localStorage.setItem(SPOKEN_CURSOR_KEY, JSON.stringify({ sessionId: sessionId, seq: seq }))
        } catch (e) {}
      }
      React.useEffect(function () {
        if (!isSister) return
